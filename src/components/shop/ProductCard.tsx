...
export default function ProductCard({ product }: Props) {
  ...
  return (
    <Link href={`/products/${product.handle}`} className="block bg-white rounded-[12px] shadow-sm hover:shadow-md border border-gray-100 transition-shadow">
      <div className="relative aspect-[4/5] w-full overflow-hidden rounded-t-[12px]">
        {product.featuredImage?.url ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={product.featuredImage.url} alt={product.featuredImage.altText || product.title} className="h-full w-full object-cover" />
        ) : (
